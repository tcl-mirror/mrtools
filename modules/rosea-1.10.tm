...
#
# This software is copyrighted 2014-2020 by G. Andrew Mangogna.
...
namespace eval ::rosea {
    ...
    namespace eval Helpers {
        ...
        set errFormats [dict create {*}{
            ...
            UNKNOWN_EVENT_TYPE     {unknown event type, \"%s\"}
        ...
        proc MultiAssignerIDAttrs {domain relationship} {
            if {[string range $domain 0 1] eq "::"} {
                set domain [string range $domain 2 end]
            }
            return [pipe {
                relvar restrictone ::rosea::Config::MultipleAssigner\
                    Domain $domain Relationship $relationship |
                relation semijoin ~ $::rosea::Config::Identifier\
                    $::rosea::Config::IdentifyingAttribute |
    ...
    namespace eval RelCmds {
        ...
        proc signalMultiAssigner {rname idvalues event args} {
            ...
            set idattrs [MultiAssignerIDAttrs $domain $relationship]
        ...
        proc deleteMultiAssigner {rname args} {
            if {[llength $args] % 2 != 0} {
                tailcall DeclError ARG_FORMAT $args
            }
            SplitRelvarName $rname domain relationship
            ...
            relvar deleteone $assignrelvar {*}$args
            return
        }
    }
    namespace eval Dispatch {
        ...
        proc SignalEvent {srcref dstset event arglist} {
            ...
            SplitRelvarName $relvar domain class
            if {[relation isnotempty [relvar restrictone ${domain}::__Arch_Event\
                Class $class Event $event]]} {
                set eventType ordinary
            } elseif {[IsPolymorphicEvent $domain $class $event]} {
                set eventType polymorphic
            } else {
                tailcall DeclError UNKNOWN_EVENT $event $class
            }
        ...
        proc IsPolymorphicEvent {domain class event} {
            return [relation isnotempty [relvar restrictone\
                ${domain}::__Arch_PolymorphicEvent Class $class Event $event]]
        }
        proc DispatchEvent {} {
            variable event_queue
            variable toc_queue
            ...
            try {
                if {[llength $event_queue] != 0} {
                    set event_queue [lassign $event_queue eventInfo]
                    TransitionEvent $eventInfo
                } elseif {[llength $toc_queue] != 0} {
                    set toc_queue [lassign $toc_queue eventInfo]
                    relvar transaction begin ;                              # <1>
                    try {
                        TransitionEvent $eventInfo
                    } on error {result opts} {
                        catch {relvar transaction end} ;                    # <2>
                        return -level 0 -options $opts $result
                    }
                } else {
                    return
                }
        ...
        proc TransitionEvent {eventInfo} {
            switch -exact -- [dict get $eventInfo type] {
                ordinary {
                    TransitionOrdinaryEvent $eventInfo
                }
                creation {
                    TransitionCreationEvent $eventInfo
                }
                polymorphic {
                    TransitionPolymorphicEvent $eventInfo
                }
                default {
                    tailcall DeclError UNKNOWN_EVENT_TYPE [dict get $eventInfo type]
                }
            }
        }
        proc TransitionOrdinaryEvent {eventInfo} {
            variable event_queue
        ...
        proc TransitionCreationEvent {eventInfo} {
            ...
            dict set eventInfo type ordinary
            dict set eventInfo dst $dstref ; # <2>
            TransitionOrdinaryEvent $eventInfo
        }
        proc TransitionPolymorphicEvent {eventInfo} {
            lassign [dict get $eventInfo dst] dstrelvar inst
            ...
            relation foreach partition $partitions {
                ...
                relation foreach dstlink $DstLinks {
                    ...
                    if {[relation isnotempty $related]} {
                        ...
                        set srcref [dict get $eventInfo src]
                        ::rosea::Trace::TracePolymorphic $srcref $event $target $dstreference $Name\
                                [dict get $eventInfo params]
                    
                        set mappedInfo $eventInfo ;                                 # <1>
                        dict set mappedInfo src $target
                        dict set mappedInfo dst $dstreference
                        if {[IsPolymorphicEvent $domain $DstClass $event]} {
                            TransitionPolymorphicEvent $mappedInfo
                        } else {
                            dict set eventInfo type ordinary
                            TransitionOrdinaryEvent $mappedInfo
                        }
                        break
                    }
                }
    ...
    namespace eval Trace {
        ...
        proc FormatInstRef {instref} {
            lassign $instref relvar inst
            return [expr {$relvar eq {} ? "()" :
                    "$relvar\([FormatNVPairs [tuple get [relation tuple $inst]]]\)"}]
        }
        ...
        proc FormatNVPairs {pairs} {
            return [pipe {
                lmap {attr val} $pairs {string cat $attr ": " $val} |
                join ~ {, }
            }]
        }
