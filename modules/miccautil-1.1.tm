...
# on Mon Jul 05 10:34:07 PDT 2021
...
::apply $__loadScript__ 960 20590
...
::oo::class create ::miccautil::model {
    ...
    method creationevents {class} {
        # log::debug "\n[relformat [relvar set TransitionPlace] TransitionPlace]"
        return [pipe {
            relvar set TransitionPlace |
            relation restrictwith ~ {$Model eq $class && $State eq "@"} |
            relation project ~ Event |
            relation list
        }]
    }
    ...
    method stateModelGraph {class_name} {
        ...
        try {
            ...
            if {[relation isnotempty $finals]} {
                $gr node insert __x__
                $gr node set __x__ activity {}
                $gr node set __x__ final false
            }
            relation foreach final_state $finals {
                relation assign $final_state Name
                $gr arc insert $Name __x__
            }
...
proc ::miccautil::graphToDot {graph {edgekeys {}} {nodekeys {}}} {
    ...
    foreach node [$graph nodes] {
        ...
        if {$node eq "@"} {
            $dot_node setattributes\
                shape circle fontcolor white fillcolor black
            set label_value {}
        } elseif {$node eq "__x__"} {
            $dot_node setattributes\
                shape doublecircle fontcolor white fillcolor black
            set label_value {}
        } else {
            set label_value [string map {_ { }} $node]
        }
        set node_attrs [$graph node keys $node]
        set node_keys [lmap nodekey $nodekeys {
            expr {$nodekey in $node_attrs ?
                "$nodekey=[$graph node get $node $nodekey]" : [continue]}
        }]
        if {$label_value ne {}} {
            set label_value [concat [list $label_value] $node_keys]
        } else {
            set label_value $node_keys
        }
        set label_value [join $label_value "\\n"]
