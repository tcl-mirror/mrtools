...
package provide oomoore 2.0

# ACTIVESTATE TEAPOT-PKG END DECLARE
# ACTIVESTATE TEAPOT-PKG END TM
# This software is copyrighted 2013 - 2014 by G. Andrew Mangogna.
...
::oo::class create ::oomoore::model {
    ...
    method receive {event args} {
        ...
        my variable currentState transitions
        ...
        if {$newState eq "CH"} {
            ...
            log::error $errmsg
            my variable chErrorHandler
            if {$chErrorHandler ne {}} {
                ::apply $chErrorHandler [self] $currentState $event
            } else {
    ...
    method cherror {lambda} {
        my variable chErrorHandler
        set oldHandler $chErrorHandler
        set chErrorHandler $lambda
        return $oldHandler
    }
    ...
    method DelayedDispatch {obj delayed details} {
        ...
        # It is possible to get here and the event has already been
        # canceled.  We lost the race excuting the "after" script.
        # If we didn't loose the race, then we can dispatch the event.
        if {[info exists delayedSignals($src,$event)]} {
    ...
    method dot {} {
        ...
        foreach state $states {
            lassign [info object definition [self] __STATE_$state] arguments body
    ...
    method State {name argList body} {
        ...
        if {$name in {IG CH}} {
            ...
            lappend states $name
            oo::objdefine [self] method __STATE_$name $argList $body
        }
    }
